...
<div class="wrapper-page">
    ...
    <div class="page-title">
        <h1><i class="icon-list"></i> View Free-Tips</h1>
    </div>
                ...
                <div class="col-md-12">
                    <div class="content_wrapper">
                        <div class="table_banner clearfix">
                            <h5 class="table_banner_title">Free-Tip List</h5>
                        ...
                        <div class="table_body text-left">
                            <table id="example" class="table table-condensed responsive table_custom" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    <?php $sr=1;foreach($frtp as $value): ?>
                                    <tr>
                                        <td>
                                           <?php echo $sr ?>
                                        </td>
                                        ...
                                        <td class="action-buttons">
                                            <button type="button" id="category" data-id="<?php echo $value->id; ?>" name="submit" class="catbutton">
                                                ...
                                                </button>  <a onclick="return confirm('Are you sure to delete this data?')" href="freetip_delet?id=<?php echo $value->id; ?>" >
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header modal-primary">
                ...
                <h4 class="modal-title" id="modal-label">Free-Tip</h4>
            </div>
            ...
            <form enctype="multipart/form-data" id="catmodal" method="post" name="catmodal" role="form" accept-charset="utf-8">
                <div class="modal-body">
                    <div class="form-group clearfix">
                        <label class="col-md-3" for="inputMaxLength">Date</label>
                        <div class="col-md-9">
                            <input class="form-control" id="date" name="date" placeholder="Date" required type="date" value=''>
                        </div>
                    </div>
                    <div class="form-group clearfix">
                        ...
                        <div class="col-md-9">
                                  <select class="form-control "  name="market_id" id="market_id">
                                    <option value="">Select Market</option>
                                        <?php foreach($marketslist as $value): ?>
                    ...
                    <div class="form-group clearfix">
                        ...
                        <div class="col-md-9">
                            <input class="form-control" id="open" name="open" placeholder="Open" required type="number" value=''>
                        </div>
                    </div>
                    <div class="form-group clearfix">
                        <label class="col-md-3" for="inputMaxLength">Close</label>
                        <div class="col-md-9">
                            <input class="form-control" id="close" name="close" placeholder="close" required type="number" value=''>
                        </div>
                    </div>
...
<script>
    $("#addcategory").on("click", function(event) {
        ...
        $.ajax({
            url: "<?php echo base_url();?>admin/save_freetip",
            type: "POST",
            dataType: 'json',
            data: {
                'id': $('#id').val(),
...
<script type="text/javascript">
    $(document).ready(function() {
        $(".catbutton").click(function(e) {
            ...
            $('#categoryform').modal('show');
            $.ajax({
                url: '<?php echo base_url();?>admin/freeTipById?id=' + iid,
                method: 'GET',
                data: '',
                dataType: 'json',
                ...
                $('#catmodal').find('[name="id"]').val(response.freetipvalue.id).end();
                $('#catmodal').find('[name="date"]').val(response.freetipvalue.date).end();
                $('#catmodal').find('[name="market_id"]').val(response.freetipvalue.market_id).end();
                $('#catmodal').find('[name="open"]').val(response.freetipvalue.open).end();
                $('#catmodal').find('[name="close"]').val(response.freetipvalue.close).end();
