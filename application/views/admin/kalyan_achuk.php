                    ...
                    <div class="content_wrapper">
                        ...
                        <div class="table_body text-left">
                            <table id="example" class="table table-condensed responsive table_custom" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    <?php $sr=1;foreach($achuklist as $value): ?>
                                    <tr>
                                        <td>
                                           <?php echo $sr ?>
                                        </td>
                                        ...
                                        <td class="action-buttons">
                                            <button type="button" id="category" data-id="<?php echo $value->id; ?>" name="submit" class="catbutton">
                                                ...
                                                </button>  <a onclick="return confirm('Are you sure to delete this data?')" href="delet_achuk?id=<?php echo $value->id; ?>" >
            ...
            <form enctype="multipart/form-data" id="catmodal" method="post" name="catmodal" role="form" accept-charset="utf-8">
                <div class="modal-body">
                    ...
                    <div class="form-group clearfix">
                        <label class="col-md-3" for="inputMaxLength">Kalyan Achuk</label>
                        <div class="col-md-9">
                            <input class="form-control" id="description" name="description" placeholder="Kalyan Achuk Data" required type="text" value=''>
                        </div>
                    </div>
                    <div class="form-group clearfix">
...
<script>
    $("#addcategory").on("click", function(event) {
        ...
        $.ajax({
            url: "<?php echo base_url();?>admin/save_achuk",
            type: "POST",
            dataType: 'json',
            data: {
                'id': $('#id').val(),
...
<script type="text/javascript">
    $(document).ready(function() {
        $(".catbutton").click(function(e) {
            ...
            $('#categoryform').modal('show');
            $.ajax({
                url: '<?php echo base_url();?>admin/kalyan_achukById?id=' + iid,
                method: 'GET',
                data: '',
                dataType: 'json',
                ...
                $('#catmodal').find('[name="id"]').val(response.achukValue.id).end();
                $('#catmodal').find('[name="description"]').val(response.achukValue.description).end();
                $('#catmodal').find('[name="status"]').val(response.achukValue.status).end();
